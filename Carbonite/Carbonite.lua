...
Nx.Whatsnew.Maps = {
	[1568657164] = { "Sept 16th 2019", "", "This is Alpha version of Carbonite Classic, all bugs/issues please report to",
		"GitHub repo: https://github.com/IrcDirk/Carbonite-Classic." },
	[1653631200] = { "May 27th 2022", "", "Fixed map path generation using FlightMasters",
	...
	[1660510987] = { "Aug 14th 2022", "", "Implemented WotLK Classic support for Pre-patch", "Updated Flight Masters data",
...
function Nx:SetupEverything()
	...
	if Nx.db.profile.General.LoginHideVer then
		Nx.prt(L["Carbonite"] .. " |cffffffff" .. Nx.VERMAJOR .. "." ..
			(Nx.VERMINOR * 10) .. " Build " .. Nx.BUILD .. " " .. L["Loading"])
	end
	...
	if ZGV and ZGV.Pointer then
		hooksecurefunc(ZGV.Pointer, "SetWaypoint", function(e, m, x, y, data, arrow)
			...
			local title = (ZGV.CurrentStep and ZGV.CurrentStep.current_waypoint_goal_num and ZGV.CurrentStep.goals) and
				ZGV.CurrentStep.goals[ZGV.CurrentStep.current_waypoint_goal_num]:GetText() or ""
...
function Nx:ADDON_LOADED()
	Nx.Loaded = true

	CarbMigr = {}
end

function Nx:UNIT_NAME_UPDATE()
	Nx.PlayerFnd = true
...
function Nx:OnUpdate_mouseover_unit()
	if Nx.Quest then
...
function Nx:UnitDTip()
	...
	if (not data or type ~= 3) then
		return
	end

	local midCur = Nx.Map:GetCurrentMapAreaID()
	local plZX, plZY = Nx.Map.GetPlayerMapPosition("player")
	local isCur = midCur and (plZX > 0 or plZY > 0)
...
function Nx:OnUpdate_battlefield_score()
...
function Nx:InitGlobal()
	...
	local hudOpts = Nx.db.profile.HUDOpts

	if not hudOpts or hudOpts.Version < Nx.VERSIONHUDOPTS then
		if hudOpts then
			Nx.prt(L["Reset old HUD options"] .. " %f", hudOpts.Version)
		end

		hudOpts = {}
		Nx.db.profile.HUDOpts = opts
		hudOpts.Version = Nx.VERSIONHUDOPTS
		-- Nx.HUD:OptsReset()
	end
...
function Nx.AuctionAssist.AuctionFrameBrowse_Update()
	...
	for n = 1, NUM_AUCTION_ITEMS_PER_PAGE do
		...
		local name, _, count, quality, _, _, minBid, minIncrement, buyoutPrice, bidAmount, _, _ = GetAuctionItemInfo("list", n)
...
function Nx.UEvents:AddTimber(name)
	if not Nx.db.profile.Guide.GatherEnabled then
		return
	end
	local mapId, x, y, level = self:GetPlyrPos()
...
function Nx.UEvents.List:Open()
	...
	win = Nx.Window:Create("NxEventsList", nil, nil, nil, nil, nil, true)
	self.Win = win
...
function Nx.UEvents:UpdateMap(upGuide)
	...
	if m then
		...
		for _, item in ipairs(Nx.CurCharacter.E) do
...
function Nx.Item.AskDeleteVV()
	...
	Nx:ShowMessage(Nx.TXTBLUE ..
		"Carbonite:\n|cffffff60" ..
		L["Delete visited vendor data?"] .. "\n" .. L["This will stop the attempted retrieval of items on login."],
		L["Delete"
		], func, L["Cancel"])
end

function Nx.Item:ShowTooltip(tooltipId, compare)
	local id = Nx.Split("^", tostring(tooltipId))
...
function Nx.NXMiniMapBut:Init()
	...
	if ok and var ~= "0" then
		Nx:ShowMessage("Profiling is on. This decreases game performance. Disable?", "Disable and Reload", self.ToggleProfiling
			, "Cancel")
	end
end
...
function Nx.NXMiniMapBut:NXOnClick(button)
...
function Nx.Proc:OnUpdate(elapsed)
	elapsed = (min(elapsed, .2) * 60) + self.TimeLeft
